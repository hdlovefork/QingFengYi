...
class User_model extends DB_model
{
     ...
     */
    public function get_token($app_key, $code)
    {
        //1.检查app_key是否存在并返回wx_key,wx_secret,tb_pid
        $app_data = $this->app_model->get_app($app_key);
        if (!$app_data) {
    ...
    private function _grant_token($wxResult, $app_data)
    {
        ...
        $uid = $this->get_user($open_id, $app_data['id']);
        // 生成令牌，准备缓存数据，写入缓存
...
//                dtk_key
     ...
     * @throws Exception
     */
    public function get_user($open_id, $app_id)
    {
